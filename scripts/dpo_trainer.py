...
def maybe_apply_chat_template(example, tokenizer, is_chat_model=True):
    if is_chat_model and "messages" in example:
...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[Union[list[int], Any, dict[str, Any]]]) -> dict[str, Any]:
        ...
        ### add the docta score
        if "chosen_docta_score" in examples[0] or "rejected_docta_score" in examples[0]:
            output['chosen_docta_score'] =  torch.tensor([example["chosen_docta_score"] for example in examples])
            output['rejected_docta_score'] =  torch.tensor([example["rejected_docta_score"] for example in examples])
            
        ## add original LLM rating score
        if "score_chosen" in examples[0] or "score_rejected" in examples[0]:
        ...
        if "reward_score_chosen" in examples[0] or "reward_score_rejected" in examples[0]:
            output['reward_score_chosen'] =  torch.tensor([example["reward_score_chosen"] for example in examples])
            output['reward_score_rejected'] =  torch.tensor([example["reward_score_rejected"] for example in examples])
...
class DPOTrainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        # ✅ 第二步：正式构建 DataLoader，不使用 shuffle
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            print("✅ use the sequentialSampler here")  # ✅ 现在会正常打印
    ...
    def dpo_loss(
        ...
        mean_chosen_logps: torch.FloatTensor,
        mean_rejected_logps: torch.FloatTensor,
        ...
        if self.loss_type == "sigmoid":
        ...
        elif self.loss_type == "beta-dpo":
            ## β-DPO (Wu et al., NeurIPS 2024): dynamic β + Gaussian data filtering
            alpha_scale = 0.6  # scaling factor (paper recommends 0.6-1.0)
            gamma_ema = 0.9    # EMA momentum
            rho = 0.8          # keep ratio (filter out 20% outlier samples)

            # Advantage gap per sample: logits = chosen_logratio - rejected_logratio
            advantage_gap = logits.detach()

            # Update running mean and std via EMA
            if not hasattr(self, '_beta_dpo_gap_mean'):
                self._beta_dpo_gap_mean = advantage_gap.mean().item()
                self._beta_dpo_gap_std = advantage_gap.std().item() + 1e-8
            else:
                self._beta_dpo_gap_mean = gamma_ema * self._beta_dpo_gap_mean + (1 - gamma_ema) * advantage_gap.mean().item()
                self._beta_dpo_gap_std = gamma_ema * self._beta_dpo_gap_std + (1 - gamma_ema) * (advantage_gap.std().item() + 1e-8)

            # Dynamic β per sample (Eq. 6 in paper)
            beta_dynamic = self.beta * (1 + alpha_scale * (advantage_gap - self._beta_dpo_gap_mean))
            beta_dynamic = beta_dynamic.clamp(min=1e-3)

            # Gaussian-weighted data filtering: keep rho fraction of samples
            gauss_weights = torch.exp(-0.5 * ((advantage_gap - self._beta_dpo_gap_mean) / self._beta_dpo_gap_std) ** 2)
            num_keep = max(int(rho * len(advantage_gap)), 1)
            keep_indices = torch.multinomial(gauss_weights, num_keep, replacement=False)
            mask = torch.zeros_like(advantage_gap)
            mask[keep_indices] = 1.0

            per_sample_losses = (
                -F.logsigmoid(beta_dynamic * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-beta_dynamic * logits) * self.label_smoothing
            )
            # Apply mask: only compute loss on selected samples
            losses = per_sample_losses * mask / mask.sum() * len(mask)
            ...
            penalty_scale = 5
            penalty = torch.maximum(torch.zeros_like(chosen_logratios), -1 * chosen_logratios)
            logits = logits - penalty_scale * penalty  # 惩罚负置信度 pair
        ...
        elif self.loss_type == "noisy-tolerant": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.1
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
            scores_diff =chosen_docta_scores - rejected_docta_scores
            sel_labels = (scores_diff < score_threshold).int()  
            
            losses = -F.logsigmoid(self.beta * logits) * chosen_docta_scores - F.logsigmoid(-self.beta * logits) * rejected_docta_scores
        
        elif self.loss_type == "noisy-tolerant-1": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.2
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
            ...
            losses = -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing * sel_labels)  - F.logsigmoid(-self.beta * logits) * self.label_smoothing * sel_labels
        
        elif self.loss_type == "noisy-tolerant-1-1": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.1
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
        ...
        elif self.loss_type == "noisy-tolerant-1-2": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.1
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
        ...
        elif self.loss_type == "noisy-tolerant-1-3": ## ours
            ### TODO: need to finished; sample-level soft label
        ...
        elif self.loss_type == "noisy-tolerant-2": ## ours
            ### TODO: need to finished; sample-level soft label
            import numpy as np
            ...
            #### stat info ##
            chosen_mean, rejected_mean = 0.547, 0.488  
            chosen_sigma, rejected_sigma = 0.2564, 0.2567  
            mean_diff, sigma_diff = chosen_mean - rejected_mean, np.sqrt(chosen_sigma**2 + rejected_sigma**2)
            
            # Calculate the confidence prob of A > B given a and b
            # confident_probs = norm.cdf(scores_diff.cpu().numpy(), loc=mean_diff, scale=sigma_diff)
            confident_probs = norm.cdf(scores_diff.cpu().float().numpy(), loc=mean_diff, scale=sigma_diff)

            confident_probs = torch.Tensor(confident_probs).to(logits.device)
            
            losses = -F.logsigmoid(self.beta * logits) * confident_probs - F.logsigmoid(-self.beta * logits) * (1-confident_probs)
            
        elif self.loss_type == "noisy-tolerant-3": ## ours
            ### TODO: need to finished; sample-level soft label
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
            normalized_chosen_scores = chosen_docta_scores / (chosen_docta_scores + rejected_docta_scores)
            normalized_rejected_scores = rejected_docta_scores / (chosen_docta_scores + rejected_docta_scores)
            
            losses = -F.logsigmoid(self.beta * logits) * normalized_chosen_scores - F.logsigmoid(-self.beta * logits) * normalized_rejected_scores
        
        elif self.loss_type == "noisy-tolerant-4": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.1
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
            
            weighted_chosen_docta_scores = (1 + chosen_docta_scores) / 2
            weighted_rejected_docta_scores = (1 + rejected_docta_scores) / 2
            
            losses = -F.logsigmoid(self.beta * logits) * weighted_chosen_docta_scores - F.logsigmoid(-self.beta * logits) * weighted_rejected_docta_scores

        elif self.loss_type == "noisy-tolerant-4-1": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.1
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
            ...
            losses = -F.logsigmoid(self.beta * logits) * weighted_chosen_docta_scores * (1 - self.label_smoothing * sel_labels) - F.logsigmoid(-self.beta * logits) * weighted_rejected_docta_scores * self.label_smoothing * sel_labels

        elif self.loss_type == "noisy-tolerant-4-2": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 0.5 
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
            ...
            normalized_chosen_ratings = (chosen_ratings - 1) / 4 ## rating score: max 5.0 min 1.0
            normalized_rejected_ratings = (rejected_ratings -1) / 4
                        
            weighted_chosen_ratings = (1 + normalized_chosen_ratings ) / 2
            weighted_rejected_ratings = (1 + normalized_rejected_ratings ) / 2

            losses = -F.logsigmoid(self.beta * logits) * weighted_chosen_ratings * (1 - self.label_smoothing * sel_labels) - F.logsigmoid(-self.beta * logits) * weighted_rejected_ratings * self.label_smoothing * sel_labels
        
        elif self.loss_type == "noisy-tolerant-4-3": ## ours
            ### TODO: need to finished; sample-level soft label
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
            score_diff = chosen_ratings - rejected_ratings
            tau = 1.0
            score_weight = torch.sigmoid(score_diff / tau)  # [B], soft weighting

            losses = -F.logsigmoid(self.beta * logits) * score_weight - F.logsigmoid(-self.beta * logits) * (1-score_weight)

        elif self.loss_type == "noisy-tolerant-4-4": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 0.5 
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
            ...
            normalized_chosen_ratings = (chosen_ratings - 1) / 9 ## rating score: max 10.0 min 1.0
            normalized_rejected_ratings = (rejected_ratings -1) / 9
        ...
        elif self.loss_type == "noisy-tolerant-4-5": ## ours
            ### TODO: need to finished; sample-level soft label
        ...
        elif self.loss_type == "noisy-tolerant-4-6": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 0.5
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
        ...
        elif self.loss_type == "noisy-tolerant-4-6-rejected": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 0.5
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
        ...
        elif self.loss_type == "noisy-tolerant-4-6-rejected-and-chosen": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 0.5
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
            ...
            sft_loss = -(mean_chosen_logps + mean_rejected_logps) / 2
            dpo_losses = (
                -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits) * self.label_smoothing
            )
        ...
        elif self.loss_type == "noisy-tolerant-4-6-threshold2": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 1.5
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
            ratings_diff = chosen_ratings - rejected_ratings
            sel_labels = (ratings_diff < rating_score_threshold).int()  
        ...
        elif self.loss_type == "noisy-tolerant-4-6-agrilla": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 0.5
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
        ...
        elif self.loss_type == "noisy-tolerant-4-7": ## ours
            ### TODO: need to finished; sample-level soft label
            rating_score_threshold = 0.5
            chosen_ratings =  torch.Tensor(batch['score_chosen']).to(logits.device) if 'score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['score_rejected']).to(logits.device) if 'score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
            ...
            dpo_losses = (-F.logsigmoid(self.beta * logits) * weighted_chosen_ratings * (1 - self.label_smoothing) - F.logsigmoid(-self.beta * logits) * weighted_rejected_ratings * self.label_smoothing)
            ...
            # print(dpo_losses)
            # print("###################")
        elif self.loss_type == "noisy-tolerant-4-8": ## ours
            ### TODO: need to finished; sample-level soft label
            chosen_ratings =  torch.Tensor(batch['reward_score_chosen']).to(logits.device) if 'reward_score_chosen' in batch.keys() else  torch.ones_like(logits).to(logits.device)
            rejected_ratings =  torch.Tensor(batch['reward_score_rejected']).to(logits.device) if 'reward_score_rejected' in batch.keys() else  torch.zeros_like(logits).to(logits.device)
            score_diff = chosen_ratings - rejected_ratings
            tau = 1.0
            score_weight = torch.sigmoid(score_diff / tau)  # [B], soft weighting
        ...
        elif self.loss_type == "noisy-tolerant-5": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.1
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
            ...
            losses = -F.logsigmoid(self.beta * logits) * chosen_docta_scores - F.logsigmoid(-self.beta * logits) * rejected_docta_scores * sel_labels
        
        elif self.loss_type == "noisy-tolerant-6": ## ours
            ### TODO: need to finished; sample-level soft label
            score_threshold = 0.1
            chosen_docta_scores =  torch.Tensor(batch['chosen_docta_score']).to(logits.device)
            rejected_docta_scores =  torch.Tensor(batch['rejected_docta_score']).to(logits.device)
            ...
            losses = (-F.logsigmoid(self.beta * logits) * chosen_docta_scores - F.logsigmoid(-self.beta * logits) * rejected_docta_scores) * sel_labels - F.logsigmoid(self.beta * logits) * (1 - sel_labels)
        
         ### Reproduce from ICLR 25 paper: https://openreview.net/pdf?id=BPgK5XW1Nb
        elif self.loss_type == "spa":
            tau = 0.1  ### data proportions that needed to be apply label smoothing loss;; default: 0.1
            epsilon = 0.5 ## lambda in Eq.(12): enhanced value
            revised_logits = (1 + epsilon) *  logratios - epsilon * ref_logratios
            
            confident_probs = F.sigmoid(self.beta * revised_logits) 
            normalized_confident_probs = F.softmax(confident_probs, dim=-1)

            # print(f"confidence probability: {normalized_confident_probs}")
            select_labels = (normalized_confident_probs < tau).int()  
            # print(f"##### less confident label: {less_confident}")
            losses = (
                -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing* select_labels)
                - F.logsigmoid(-self.beta * logits) * self.label_smoothing * select_labels
            )
            ...
            sft_losses = -mean_chosen_logps
            self.gamma_beta_ratio = 0.5 # for llama-3-8b
            ...
            losses = sel_labels * sft_losses + (1 - sel_labels) * simpo_losses
            ...
            sft_losses = -mean_chosen_logps
            ipo_losses = (logits - 1 / (2 * self.beta)) ** 2
            # losses = -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing * sel_labels) - F.logsigmoid(-self.beta * logits) * weighted_rejected_ratings * self.label_smoothing * sel_labels
            
            losses = sel_labels * sft_losses + (1 - sel_labels) * ipo_losses
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor]]):
        ...
        if self.loss_type == "sigmoid-mean": ###check the dpo-mean logps
            all_logps = all_logps / loss_mask.sum(-1)
        ...
        all_logps_temp =  all_logps / loss_mask.sum(-1)
        output["mean_chosen_logps"] = all_logps_temp[:num_examples]
        output["mean_rejected_logps"] = all_logps_temp[num_examples:]
    ...
    def get_batch_loss_metrics(
        ...
        losses, chosen_rewards, rejected_rewards = self.dpo_loss(
            batch, model_output["chosen_logps"], model_output["rejected_logps"], ref_chosen_logps, ref_rejected_logps, model_output["mean_chosen_logps"], model_output["mean_rejected_logps"]
        )
